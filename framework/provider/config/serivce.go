...
type JinConfig struct {
	...
	confMaps map[string]interface{} //配置文件结构，key文件名
	confRaws map[string][]byte      //配置文件的原始信息
}

// 读取某个配置文件
func (conf *JinConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *JinConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
func NewJinConfig(params ...interface{}) (interface{}, error) {
	...
	for _, file := range files {
		filename := file.Name()
		err := jinConf.loadConfigFile(envFolder, filename)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						jinConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						jinConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path获取某个元素
func (conf *JinConfig) find(key string) interface{} {
	conf.lock.RLock()
